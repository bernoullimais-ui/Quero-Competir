...
async function run() {
  const { data } = await supabase.from('athlete_subscriptions')
    .select('id, athlete_name, tournament_id')
    .or("athlete_name.ilike.%Artur magnavita%,athlete_name.ilike.%Carlos José Vidal Euzébio%");
