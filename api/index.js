...
router2.post("/:id/seed-athletes", async (req, res) => {
  try {
    const tournamentId = req.params.id;
    const { count = 100 } = req.body;
    const supabase = getSupabaseAdmin();
    const { data: categories, error: catErr } = await supabase.from("tournament_categories").select("*").eq("tournament_id", tournamentId);
    ...
    for (let i = 0; i < count; i++) {
      ...
      const name = `${fn} ${ln}`;
      const birthYear = category.birth_year_max ? category.birth_year_max : category.birth_year_min ? category.birth_year_min : 1995;
      ...
      const { data, error } = await supabase.from("athlete_subscriptions").insert(insertData).select().maybeSingle();
      if (data) {
        inserted.push(data);
      }
    }
    res.json({
      success: true,
      message: `${inserted.length} participantes fict\xEDcios inseridos com sucesso em ${categories.length} categorias!`,
      count: inserted.length
    });
  } catch (err) {
    console.error("Erro ao gerar participantes fict\xEDcios:", err);
    res.status(500).json({ error: err.message });
  }
});
