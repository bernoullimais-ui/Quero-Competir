  ...
  try {
    ...
    if (method === "pix") {
      ...
      if ((charge?.status === "failed" || !transaction?.qr_code) && splitRules && orderPayload.payments[0].split) {
        ...
        console.warn("[Pagar.me Institution Pix Charge Failed] Tentando criar cobran\xE7a Pix sem split. Erro gateway:", gwErr);
        delete orderPayload.payments[0].split;
        pgOrder = await callPagarMe("/orders", "POST", orderPayload);
      ...
      if (!qrCode) {
        const errMsg = charge?.last_transaction?.gateway_response?.errors?.[0]?.message || "O gateway Pagar.me n\xE3o retornou a chave Pix para esta transa\xE7\xE3o.";
...
<!--EVENT_TIME:${timeVal.trim()}-->`;
    ...
    if (finalDesc !== void 0) updateObj.description = finalDesc;
    ...
    if (timeVal !== void 0) updateObj.event_time = timeVal;
    let { data, error } = await supabase.from("tournaments").update(updateObj).eq("id", req.params.id).select().maybeSingle();
    if (error) {
      delete updateObj.location;
      delete updateObj.event_time;
      const { data: retryData, error: retryErr } = await supabase.from("tournaments").update(updateObj).eq("id", req.params.id).select().maybeSingle();
...
router2.post("/:id/categories/reorder", async (req, res) => {
  ...
  try {
    const supabase = getSupabaseAdmin();
    const { data: cats, error: fetchErr } = await supabase.from("tournament_categories").select("*").eq("tournament_id", req.params.id);
    ...
    if (cats && cats.length > 0) {
      for (let i = 0; i < orderedIds.length; i++) {
        ...
        if (existingCat) {
          ...
          await supabase.from("tournament_categories").update({ rules_config: updatedConfig }).eq("id", catId);
        }
      }
    }
...
function mapSubToFrontend(dbSub) {
  ...
  return {
    ...
    additionalData: add,
    athleteFee: add.athleteFee !== void 0 ? Number(add.athleteFee) : add.registeredFee !== void 0 ? Number(add.registeredFee) : null,
    totalFee: add.totalFee !== void 0 ? Number(add.totalFee) : null,
...
async function getSubscriptionSettings(idOrSlug) {
  try {
    ...
    let showBrackets = true;
    if (data?.registration_config?.show_brackets_publicly !== void 0) {
      showBrackets = !!data.registration_config.show_brackets_publicly;
    } else if (data?.show_brackets_publicly !== void 0) {
      showBrackets = !!data.show_brackets_publicly;
    } else if (tData3?.rules_config?.show_brackets_publicly !== void 0) {
      showBrackets = !!tData3.rules_config.show_brackets_publicly;
    } else {
      const db2 = loadDb();
      const raw = db2.settings[tournamentId2] || db2.settings[idOrSlug];
      if (raw && raw.showBracketsPublicly !== void 0) {
  ...
  if (rawSettings && rawSettings.showBracketsPublicly !== void 0) {
    showBracketsFallback = !!rawSettings.showBracketsPublicly;
  } else if (tData2?.rules_config?.show_brackets_publicly !== void 0) {
    showBracketsFallback = !!tData2.rules_config.show_brackets_publicly;
  }
  if (rawSettings) {
  ...
  try {
    ...
    if (payload.showBracketsPublicly !== void 0) {
      try {
        ...
        const currentRules = tour?.rules_config || {};
        await supabase.from("tournaments").update({
          rules_config: {
            ...currentRules,
            show_brackets_publicly: !!payload.showBracketsPublicly
          }
        }).eq("id", tournamentId);
      } catch (err) {
        console.error("Error updating rules_config in tournaments:", err);
      }
    }
    let { error } = await supabase.from("tournament_subscription_settings").upsert({
      tournament_id: tournamentId,
      ...dbPayload
    });
    if (error) {
      console.error("[saveSubscriptionSettings] Supabase upsert error:", error.message);
      if (error.message?.includes("show_brackets_publicly")) {
        const { show_brackets_publicly, ...sansCol } = dbPayload;
        await supabase.from("tournament_subscription_settings").upsert({
          tournament_id: tournamentId,
          ...sansCol
        });
      }
...
router2.post("/:id/self-register", async (req, res) => {
  try {
    ...
    const targetCategoryIds = Array.isArray(rawCategoryIds) && rawCategoryIds.length > 0 ? rawCategoryIds : categoryId ? [categoryId] : [];
    ...
    if (maxEvents > 0 && targetCategoryIds.length > maxEvents) {
      return res.status(400).json({ error: `O limite m\xE1ximo \xE9 de ${maxEvents} prova(s) por participante.` });
    }
    for (const catId of targetCategoryIds) {
      ...
      if (!category) return res.status(400).json({ error: `Categoria n\xE3o encontrada (${catId}).` });
      if (birthDate && (category.birth_year_min || category.birth_year_max)) {
        ...
        if (category.birth_year_min && birthYear < category.birth_year_min) {
          return res.status(400).json({ error: `Atleta n\xE3o eleg\xEDvel para ${category.name} (nascido antes de ${category.birth_year_min}).` });
        }
        if (category.birth_year_max && birthYear > category.birth_year_max) {
          return res.status(400).json({ error: `Atleta n\xE3o eleg\xEDvel para ${category.name} (nascido ap\xF3s ${category.birth_year_max}).` });
        }
      }
      const { data: existingSub } = await supabase.from("athlete_subscriptions").select("id").eq("tournament_id", tournamentId).eq("category_id", catId).eq("document", docNum).maybeSingle();
      if (existingSub) {
        return res.status(409).json({ error: `J\xE1 existe uma inscri\xE7\xE3o na prova "${category.name}" para este documento.` });
      }
    }
    ...
    for (const catId of targetCategoryIds) {
      ...
      const { data: sub, error: subErr } = await supabase.from("athlete_subscriptions").insert(insertData).select().maybeSingle();
      if (subErr) {
        if (subErr.message?.includes("created_by")) {
          delete insertData.created_by;
          insertData.additional_data.created_by = guardianAccountId;
          const { data: retrySub, error: retryErr } = await supabase.from("athlete_subscriptions").insert(insertData).select().maybeSingle();
          if (retryErr) throw retryErr;
...
router2.post("/public/athlete-subscription/:subId/pay", async (req, res) => {
  ...
  try {
    ...
    if (method === "pix") {
      ...
      if ((charge?.status === "failed" || !transaction?.qr_code) && splitRules && orderPayload.payments[0].split) {
        const gwErr = charge?.last_transaction?.gateway_response?.errors?.[0]?.message || charge?.status || "failed";
        console.warn("[Pagar.me Pix Charge Failed] Tentando criar cobran\xE7a Pix sem split. Erro gateway:", gwErr);
        delete orderPayload.payments[0].split;
        pgOrder = await callPagarMe2("/orders", "POST", orderPayload);
        charge = pgOrder.charges?.[0];
        transaction = charge?.last_transaction || charge?.transactions?.[0];
  ...
  try {
    ...
    if (sub.checkedInAt) {
      return res.json({
        ...
        message: `\u26A0\uFE0F Check-in j\xE1 realizado anteriormente \xE0s ${new Date(sub.checkedInAt).toLocaleTimeString("pt-BR", { timeZone: "America/Sao_Paulo" })}`,
...
var BANK_DATA_FILE = path5.join(process.cwd(), "src", "backend", "data", "bank_details.json");
function loadBankDataDb() {
  try {
    const dir = path5.dirname(BANK_DATA_FILE);
    if (!fs5.existsSync(dir)) fs5.mkdirSync(dir, { recursive: true });
    if (!fs5.existsSync(BANK_DATA_FILE)) {
      fs5.writeFileSync(BANK_DATA_FILE, "{}", "utf-8");
    }
    return JSON.parse(fs5.readFileSync(BANK_DATA_FILE, "utf-8"));
  } catch (e) {
    return {};
  }
}
function saveBankDataDb(data) {
  try {
    const dir = path5.dirname(BANK_DATA_FILE);
    if (!fs5.existsSync(dir)) fs5.mkdirSync(dir, { recursive: true });
    fs5.writeFileSync(BANK_DATA_FILE, JSON.stringify(data, null, 2), "utf-8");
  } catch (e) {
  }
}
function getLocalBankRecord(id) {
  ...
  for (const k of keys) {
    if (k !== "__GLOBAL__" && db[k] && (db[k].platformFeePercent !== void 0 || db[k].holderName || db[k].pagarmeRecipientId)) {
      return db[k];
    }
  }
  return {};
...
function getFinDataFromOrg(org) {
  const fin = {
  ...
  if (!org) return fin;
  if (org.platform_fee_percent !== void 0 && org.platform_fee_percent !== null) {
  ...
  if (org.description && typeof org.description === "string" && org.description.includes("__FIN_DATA__:")) {
    try {
      ...
      if (parsed) {
        if (parsed.platformFeePercent !== void 0) fin.platformFeePercent = Number(parsed.platformFeePercent);
...
function embedFinDataInDescription(currentDescription, finDataPatch) {
  ...
  try {
    ...
    if (recipientId && process.env.PAGARME_SECRET_KEY) {
      try {
        ...
        if (cleanKey) {
          pagarmeDetails = await callPagarMe3(`/recipients/${recipientId}`, "GET", null);
          if (pagarmeDetails && pagarmeDetails.status) {
            ...
            if (org && liveStatus !== org.pagarme_recipient_status) {
              ...
              await supabase.from("organizations").update({ pagarme_recipient_status: liveStatus, description: newDesc }).eq("id", org.id);
            }
          }
        }
      } catch (err) {
    ...
    let fee = localData.platformFeePercent !== void 0 ? Number(localData.platformFeePercent) : finData.platformFeePercent;
...
router6.post("/organizations/:id/create-recipient", requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (process.env.PAGARME_SECRET_KEY) {
      if (!forceCreateNew && existingRecipientId && existingRecipientId.startsWith("re_") && !existingRecipientId.startsWith("re_simulated_")) {
        try {
          ...
          try {
            const pgRecipient = await callPagarMe3(`/recipients/${existingRecipientId}`, "PUT", {
              name: holderName,
    ...
    try {
      const { error: updateErr } = await supabase.from("organizations").upsert(dbPayload);
      if (updateErr) {
        console.warn("Aviso ao salvar organiza\xE7\xE3o no Supabase (atualizando apenas description):", updateErr.message);
        await supabase.from("organizations").update({ description: newDesc }).eq("id", orgIdToUse);
      }
    } catch (e) {
      console.warn("Exce\xE7\xE3o Supabase ao salvar recebedor:", e.message);
    }
...
router6.post("/organizations/:id/link-recipient", requireAuth, async (req, res) => {
  const { id } = req.params;
  const { recipientId } = req.body;
  if (!recipientId || typeof recipientId !== "string" || !recipientId.trim()) {
    return res.status(400).json({ error: "Informe um ID de Recebedor do Pagar.me v\xE1lido (ex: re_...)." });
  }
  ...
  try {
    ...
    if (process.env.PAGARME_SECRET_KEY) {
      try {
        const pgRecipient = await callPagarMe3(`/recipients/${cleanRecipientId}`, "GET", null);
        if (pgRecipient) {
          recipientStatus = pgRecipient.status || "active";
    ...
    try {
      const { error: updateErr } = await supabase.from("organizations").upsert(dbPayload);
      if (updateErr) {
        await supabase.from("organizations").update({ description: newDesc, pagarme_recipient_id: cleanRecipientId, pagarme_recipient_status: recipientStatus }).eq("id", orgIdToUse);
      }
    } catch (e) {
      console.warn("Exce\xE7\xE3o Supabase ao vincular recebedor:", e.message);
    }
    saveBankRecord(orgIdToUse, {
      pagarmeRecipientId: cleanRecipientId,
      pagarmeRecipientStatus: recipientStatus,
    ...
    try {
      ...
      for (const acc of organizerAccounts) {
        ...
        if (!resultMap.has(targetId)) {
          ...
          const fee = localData.platformFeePercent !== void 0 ? Number(localData.platformFeePercent) : 10;
...
router6.patch("/admin/organizations/:id/fee", requireAuth, async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.warn("Exce\xE7\xE3o ao salvar fee no Supabase:", e.message);
    }
...
router8.get("/public-tournaments", async (_req, res) => {
  try {
    const supabase = getSupabaseAdmin();
    const { data, error } = await supabase.from("tournaments").select("id, name, start_date, end_date, status, logo_url, owner_id").order("start_date", { ascending: false }).limit(9);
    if (error) throw error;
    return res.json(data || []);
  } catch (err) {
