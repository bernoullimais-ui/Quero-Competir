...
export default function LiveVenueScoreboard() {
  const { tournamentId, venueId } = useParams();
  ...
  const fetchEvents = async () => {
    if (!match?.id) return;
    try {
      const res = await fetch(`/api/tournaments/matches/${match.id}/events`);
      if (res.ok) {
  ...
  const fetchScore = async () => {
    try {
      const res = await fetch(`/api/tournaments/${tournamentId}/venues/${venueId}/live`);
      const data = await res.json();
  ...
  if (!match) return (
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center text-white">
      ...
      <h2 className="text-2xl font-black uppercase tracking-widest text-slate-400">Próxima partida em breve...</h2>
    </div>
  );
