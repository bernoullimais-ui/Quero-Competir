...
router.post("/:id/seed-athletes-v2", async (req, res) => {
  try {
    const tournamentId = req.params.id;
    const supabase = getSupabaseAdmin();

    const clubs = [
      "Clube Fluir de Natação",
      "Academia Tubarão Aquática",
      "Clube Regatas Baiano",
      "Associação Aquática Salvador",
      "Esporte Clube Vitória Aquática",
      "Natação & Cia"
    ];

    // 1. Atualizar inscrições existentes sem clube
    ...
    if (existingSubs && existingSubs.length > 0) {
      for (const sub of existingSubs) {
        ...
        if (!addData.club_name && !addData.institution_name) {
          ...
          await supabase
            .from("athlete_subscriptions")
            .update({ additional_data: addData })
            .eq("id", sub.id);
        }
      }
    }
    ...
    const firstNames = [
      ...
      "Caio", "Daniel", "Hugo", "Igor", "João", "Kevyn", "Luan", "Miguel", "Murilo", "Otávio",
      "Paula", "Renata", "Sabrina", "Tainá", "Vitória", "Yasmin", "Bianca", "Clara", "Debora", "Evelyn"
    ];
    ...
    for (const cat of categories) {
      ...
      for (let i = 0; i < needed; i++) {
        ...
        const birthYear = cat.birth_year_max 
          ? cat.birth_year_max 
          : (cat.birth_year_min ? cat.birth_year_min : 1995);
    ...
    res.json({
      success: true,
      message: `Clubes atribuídos e +${totalNew} novos atletas inseridos! Cada prova agora possui em média 12 inscritos.`,
      newCount: totalNew
    });
  } catch (err: any) {
